...
- (UIImage *)bgImageForPage:(NSUInteger)idx {
    return [self pageForIndex:idx].bgImageView.image;
}
...
- (void)turnPage
{
    ...
    [self setCurrentPageIndex:self.currentPageIndex animated:YES];
}
// 定时器 绑定的方法
- (void)runTimePage
{
    int page = _pageControl.currentPage; // 获取当前的page
    page++;
    page = page > [_pages count] ? 0 : page ;
    _pageControl.currentPage = page;
    [self turnPage];
}

- (void)scrollPageWithDuration:(CGFloat)duration {
    
	[NSTimer scheduledTimerWithTimeInterval:duration target:self selector:@selector(runTimePage) userInfo:nil repeats:YES];
}
